...
let loggingInit = () => {
    ...
    logger.log('info', "START: LogFile " + config.telemetry.log_file);
    defer.resolve();
...
let uploadTelemetryFile = (fileName, jwt, endpoint = config.telemetry.sync_api.url) => {
    let defer = q.defer();
...
let uploadTelemetryDirectory = () => {
    ...
    let defer = q.defer();
    let telDir = config.telemetry.src_dir;
