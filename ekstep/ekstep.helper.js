...
let registerEkStepApp = () => {
    ...
    let initialToken = config.telemetry.app_register_api.token;
    let callableUrl = config.telemetry.app_register_api.url;
    let headers = {
...
https://diksha.gov.in/api/api-manager/v1/consumer/mobile_device_openrap/credential/register \
    ...
    -d '{
        "id": "diksha.data_exhaust_dataset_service",
        "ver": "2.0",
        "ets": 1479546952839,
        "params": {
            "requesterId": "",
...
let registerEkStepDevice = (token) => {
    ...
    const pattern = 'YYYY-MM-DD HH:mm:ss:SSSZZ';
    const timestamp = getTimestamp(pattern);
    let callableUrl = config.telemetry.device_register_api.url;
    let headers = {
...
let generateOriginalJWTs = () => {
    ...
    registerEkStepApp()
        .then(value => {
            key = value.key;
            secret = value.secret;
            console.log(`${config.plugin_name} app registered`);
        ...
        .then(value => {
            key = value.key;
            secret = value.secret;
            console.log(`${config.plugin_name} device registered`);
            return generateJwt(key, secret);
        })
        .then(value => {
            token = value.token;
